...
export function runSentryPhase0Mock(scenario: SentryMockScenario): void {
  ...
  const envLabel = import.meta.env.DEV ? 'dev' : 'production-mock';
  ...
  switch (scenario) {
    case 'render':
      ...
      console.info(`[sentry-mock] render → Sentry 전송됨 (${envLabel})`);
      break;
    ...
    case 'report-500':
      ...
      console.info(`[sentry-mock] report-500 → Sentry 전송됨 (${envLabel})`);
      break;
    ...
    case 'stream-parse':
      ...
      console.info(`[sentry-mock] stream-parse → Sentry 전송됨 (${envLabel})`);
      break;
    ...
    case 'voice-timeout':
      ...
      console.info(`[sentry-mock] voice-timeout → Sentry 전송됨 (${envLabel})`);
      break;
